...
function Get-GenesisInboxHeader([string]$Path) {
    # An inbox entry's header is its `key: value` lines before the first blank line; first key wins.
    # A file holding a NUL byte is binary (evidence) and has no header: $null.
    $bytes = [IO.File]::ReadAllBytes($Path)
    if ([Array]::IndexOf($bytes, [byte]0) -ge 0) { return $null }
    $header = [ordered]@{}
    foreach ($line in [Text.UTF8Encoding]::new($false).GetString($bytes) -split '\r?\n') {
        if ($line -eq '') { break }
        if ($line -match '^([a-z][a-z_-]*):\s*(.*?)\s*$' -and -not $header.Contains($Matches[1])) { $header[$Matches[1]] = $Matches[2] }
    }
    $header
}
...
# Synopsis: every audit/inbox/ name is the SHA-256 of its bytes (B10.4); every evidence: path exists. Not in default.
task VerifyInbox {
    ...
    foreach ($f in $files) {
        $hash = (Get-FileHash -LiteralPath $f.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
        if ($f.Name -cne $hash) { $lines.Add("INBOX_NAME audit/inbox/$($f.Name) sha256=$hash (B10.4)") }
        # evidence: is a comma-separated list of repo-relative paths.
        $header = Get-GenesisInboxHeader $f.FullName
        if (-not $header -or -not $header.Contains('evidence')) { continue }
        foreach ($e in @($header['evidence'] -split '\s*,\s*' | Where-Object { $_ })) {
            if (-not (Test-Path -LiteralPath (Join-Path $BuildRoot $e) -PathType Leaf)) { $lines.Add("INBOX_EVIDENCE audit/inbox/$($f.Name) evidence=$e missing") }
        }
    }
    foreach ($l in $lines) { Write-Build Red $l }
...
# Synopsis: regenerate .agents/inbox.md from audit/inbox/ headers; red if the committed copy was stale. Not in default.
task InboxIndex {
    $inbox = Join-Path $BuildRoot 'audit/inbox'
    if (-not (Test-Path -LiteralPath $inbox -PathType Container)) { throw 'InboxIndex: missing input audit/inbox/' }
    $out = [Text.StringBuilder]::new()
    [void]$out.Append("# audit/inbox index`n`n")
    [void]$out.Append("Generated by ``Invoke-Build InboxIndex`` from ``audit/inbox/``. Do not edit. Entry bytes: ``https://raw.githubusercontent.com/JerryBalmer1/ClaudeGenesisTemp/main/audit/inbox/<hash>```n`n")
    [void]$out.Append("| hash | attributed-to | captured-at | context |`n|---|---|---|---|`n")
    foreach ($f in Get-ChildItem -Path $inbox -File -Force | Where-Object Name -NE '.gitkeep' | Sort-Object Name) {
        $header = Get-GenesisInboxHeader $f.FullName
        $cells = if ($null -eq $header) { '-', '-', '(binary)' } else {
            # Keys as entries have written them; the first entries used author: and captured_at:.
            $by = @($header['attributed-to'], $header['author']) | Where-Object { $_ } | Select-Object -First 1
            $at = @($header['captured-at'], $header['captured_at']) | Where-Object { $_ } | Select-Object -First 1
            $ctx = [string]$header['context']
            ($by ?? '-'), ($at ?? '-'), ($ctx ? $ctx.Substring(0, [Math]::Min(80, $ctx.Length)) : '-')
        }
        [void]$out.Append("| $($f.Name) | $(($cells | ForEach-Object { $_.Replace('|', '\|') }) -join ' | ') |`n")
    }
    $path = Join-Path $BuildRoot '.agents/inbox.md'
    $new = [Text.UTF8Encoding]::new($false).GetBytes($out.ToString())
    if ((Test-Path -LiteralPath $path -PathType Leaf) -and [Linq.Enumerable]::SequenceEqual([IO.File]::ReadAllBytes($path), $new)) {
        Write-Build Green 'InboxIndex: .agents/inbox.md is current'
        return
    }
    [IO.File]::WriteAllBytes($path, $new)
    throw 'InboxIndex: .agents/inbox.md was stale; regenerated, commit it'
}
